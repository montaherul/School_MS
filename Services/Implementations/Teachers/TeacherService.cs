...
public class TeacherService : ITeacherService
{
    ...
    public async Task<int> CreateAsync(TeacherUpsertDto dto, string createdBy, CancellationToken ct = default)
    {
        ...
        if (await _userRepository.AnyAsync(u => u.Email == email, ct))
        {
        ...
        {
            dto.ProfilePicturePath = await SaveFileAsync(dto.ProfilePicture, "teachers/profiles", ct);
        }
        if (dto.PassportFile != null && dto.PassportFile.Length > 0)
        {
            dto.PassportPath = await SaveFileAsync(dto.PassportFile, "teachers/documents", ct);
        }
        if (dto.NationalIdFile != null && dto.NationalIdFile.Length > 0)
        {
            dto.NationalIdPath = await SaveFileAsync(dto.NationalIdFile, "teachers/documents", ct);
        }
        ...
        var teacher = new Teacher
        {
            ...
            NationalIdPath = dto.NationalIdPath,
    ...
    public async Task UpdateAsync(TeacherUpsertDto dto, string updatedBy, CancellationToken ct = default)
    {
        ...
        if (!string.IsNullOrEmpty(dto.PassportPath)) teacher.PassportPath = dto.PassportPath;
        if (!string.IsNullOrEmpty(dto.NationalIdPath)) teacher.NationalIdPath = dto.NationalIdPath;
        ...
        if (dto.ProfilePicture != null && dto.ProfilePicture.Length > 0)
        {
            if (!string.IsNullOrEmpty(teacher.ProfilePicturePath)) DeleteFile(teacher.ProfilePicturePath);
            teacher.ProfilePicturePath = await SaveFileAsync(dto.ProfilePicture, "teachers/profiles", ct);
        }
        if (dto.PassportFile != null && dto.PassportFile.Length > 0)
        {
            if (!string.IsNullOrEmpty(teacher.PassportPath)) DeleteFile(teacher.PassportPath);
            teacher.PassportPath = await SaveFileAsync(dto.PassportFile, "teachers/documents", ct);
        }
        if (dto.NationalIdFile != null && dto.NationalIdFile.Length > 0)
        {
        ...
        if (teacher.UserId.HasValue)
        {
            ...
            if (user != null)
            {
                if (await _userRepository.AnyAsync(u => u.Email == dto.EmailAddress && u.Id != user.Id, ct)) throw new Exception("Email already exists.");
