...
public class FeeStructureService : IFeeStructureService
{
    ...
    public async Task<PagedResult<FeeStructureListItemDto>> GetPagedAsync(int page, int pageSize, string? search, CancellationToken cancellationToken = default)
    {
        ...
        var (items, totalCount) = await _structureRepository.GetListByStoredProcedureAsync(page, pageSize, search, cancellationToken);
    ...
    {
        var entity = await _unitOfWork.Repository<FeeStructure>().FirstOrDefaultAsync(x => x.Id == dto.Id && !x.IsDeleted, cancellationToken) ?? throw new InvalidOperationException("FeeStructure not found.");
    ...
    {
        var entity = await _unitOfWork.Repository<FeeStructure>().FirstOrDefaultAsync(x => x.Id == id && !x.IsDeleted, cancellationToken) ?? throw new InvalidOperationException("FeeStructure not found.");
