...
LEFT JOIN Guardians g WITH(NOLOCK) ON sg.GuardianId = g.Id
    WHERE g.Id IS NULL;
...
FROM StudentGuardians sg WITH(NOLOCK)
LEFT JOIN Students s WITH(NOLOCK) ON sg.StudentId = s.Id
    WHERE s.Id IS NULL;
...
LEFT JOIN Guardians g WITH(NOLOCK) ON u.Id = g.UserId
...
LEFT JOIN Guardians g WITH(NOLOCK) ON gn.GuardianId = g.Id
    WHERE g.Id IS NULL;
...
LEFT JOIN StudentGuardians sg WITH(NOLOCK) ON g.Id = sg.GuardianId AND sg.IsDeleted = 0
