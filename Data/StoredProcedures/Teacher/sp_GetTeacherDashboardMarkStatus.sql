...
INNER JOIN Exams e WITH(NOLOCK) ON e.Id = me.ExamId AND e.IsDeleted = 0
INNER JOIN Subjects sub WITH(NOLOCK) ON sub.Id = me.SubjectId AND sub.IsDeleted = 0
INNER JOIN SchoolClasses c WITH(NOLOCK) ON c.Id = me.ClassId AND c.IsDeleted = 0
INNER JOIN Sections sec WITH(NOLOCK) ON sec.Id = me.SectionId AND sec.IsDeleted = 0
