...
public class SectionController : Controller
{
    ...
    public async Task<IActionResult> CreateEdit(int? id)
    {
        if (id.HasValue && id > 0)
        {
            ...
            var vm = new SectionViewModel { Id = dto.Id,SchoolClassId = dto.SchoolClassId,Name = dto.Name,            };
            return View(vm);
        }
    ...
    public async Task<IActionResult> CreateEdit(SectionViewModel vm)
    {
        ...
        if (vm.IsEditMode) { await _service.UpdateAsync(vm, userId); TempData["SuccessMessage"] = "Section updated successfully."; }
        else { await _service.CreateAsync(vm, userId); TempData["SuccessMessage"] = "Section created successfully."; }
