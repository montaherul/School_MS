...
{
    private const string ViewPath = "~/Views/Fee/FeeLedger";
    ...
    [HttpGet]
    [RequirePermission("FeeLedger.Read")]
    public async Task<IActionResult> GetList(int page = 1, int pageSize = 10, string? search = null, int? studentId = null, int? transactionType = null)
    {
        if (_security.HasStudentRole(User)) studentId = _security.GetCurrentStudentId(User);
        var result = await _service.GetPagedAsync(page, pageSize, search, studentId, transactionType);
